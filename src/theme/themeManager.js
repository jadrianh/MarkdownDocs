// Mismo guard defensivo que ya usa state.js -- antes themeManager.js accedía
// a localStorage directamente sin protección, inconsistente con el resto
// de la base de código y potencialmente frágil en entornos sin `window`.
const getStored = (key, fallback) => {
    try {
        ...
        // Fallback seguro para entornos de test/node
    }
    return fallback;
};
...
export const ThemeManager = {
    config: {
        mode: getStored('themeMode', 'light'),
        colorRGB: getStored('themeColor', '79 70 229'),
        editorFontFamily: getStored('editorFontFamily', "'JetBrains Mono', monospace"),
        editorFontSize: parseInt(getStored('editorFontSize', '14'), 10),
        previewFontFamily: getStored('previewFontFamily', "'Hanken Grotesk', sans-serif"),
        previewFontSize: parseInt(getStored('previewFontSize', '15'), 10),
    },
    ...
    setupEventListeners() {
        ...
        editorFontFamilySelect?.addEventListener('change', () => {
            ...
            setStored('editorFontFamily', this.config.editorFontFamily);
            this.applyEditorTypography(this.config.editorFontFamily, this.config.editorFontSize);
        });
        ...
        previewFontFamilySelect?.addEventListener('change', () => {
            ...
            setStored('previewFontFamily', this.config.previewFontFamily);
            this.applyPreviewTypography(this.config.previewFontFamily, this.config.previewFontSize);
        });
        ...
        previewFontSizeRange?.addEventListener('input', () => {
            ...
            setStored('previewFontSize', size);
    ...
    applyColor(rgbString) {
        ...
        // Calcular contraste óptimo para texto e iconos sobre el color de acento (WCAG AA)
        const parts = rgbString.split(' ').map(Number);
        if (parts.length === 3) {
            const [r, g, b] = parts;
            const luminance = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
            const onPrimary = luminance > 0.6 ? '#18181b' : '#ffffff';
            document.documentElement.style.setProperty('--on-primary', onPrimary);
        }

        // Actualizar indicador visual en el botón de tema de la barra de navegación
        const indicator = document.getElementById('themeAccentIndicator');
        if (indicator) {
            indicator.style.backgroundColor = `rgb(${rgbString})`;
        }

        this.config.colorRGB = rgbString;
        setStored('themeColor', rgbString);
    },
